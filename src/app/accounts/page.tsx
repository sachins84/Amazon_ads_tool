  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0d1117" }}>
      ...
      <main style={{ padding: "28px", maxWidth: 900, margin: "0 auto" }}>
        ...
        {/* Account list */}
        {loading ? (
          <div style={{ color: "#555f6e", fontSize: 13 }}>Loading accounts…</div>
        ) : accounts.length === 0 ? (
      ...
      {deleteConfirm && (
        <>
          ...
          <div style={{
            ...
            background: "#161b27", border: "1px solid #2a3245", borderRadius: 12,
            width: 360, padding: 24, zIndex: 300,
          }}>
            <h3 style={{ fontSize: 15, fontWeight: 700, color: "#e2e8f0", marginBottom: 8 }}>Remove account?</h3>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <button onClick={() => setDeleteConfirm(null)} style={{ flex: 1, padding: "8px", borderRadius: 7, background: "#1c2333", border: "1px solid #2a3245", color: "#8892a4", cursor: "pointer", fontSize: 13 }}>Cancel</button>
      ...
      {toast && (
        <div style={{
          ...
          background: "#1c2333", border: `1px solid ${toast.ok ? "#2a3245" : "rgba(239,68,68,0.3)"}`,
    ...
    <div style={{
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <span style={{ fontSize: 14, fontWeight: 600, color: "#e2e8f0" }}>{account.name}</span>
          <span style={{
        ...
        <div style={{ fontSize: 11, color: "#555f6e", marginTop: 3 }}>
...
function AccountForm({ form, setForm, editId, saving, showAdvanced, setShowAdvanced, onSave, onCancel, onOAuth }: {
  ...
  return (
    <div style={{ background: "#161b27", border: "1px solid #2a3245", borderRadius: 10, padding: "24px", marginTop: 8 }}>
      ...
      {showAdvanced && (
        <div style={{ marginTop: 12, padding: "16px", background: "#1c2333", borderRadius: 8, border: "1px solid #2a3245" }}>
      ...
      <div style={{ display: "flex", gap: 8, marginTop: 20 }}>
        <button onClick={onCancel} style={{ flex: 1, padding: "9px", borderRadius: 7, background: "#1c2333", border: "1px solid #2a3245", color: "#8892a4", cursor: "pointer", fontSize: 13 }}>Cancel</button>
...
function EmptyState({ onAdd }: { onAdd: () => void }) {
  return (
    <div style={{ textAlign: "center", padding: "60px 20px", color: "#555f6e" }}>
      <div style={{ fontSize: 40, marginBottom: 12 }}>🏷</div>
      <p style={{ fontSize: 14, color: "#8892a4", marginBottom: 16 }}>No accounts connected yet.</p>
...
function Divider({ label }: { label: string }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 10, margin: "4px 0 16px" }}>
      <span style={{ fontSize: 10, fontWeight: 600, color: "#555f6e", textTransform: "uppercase", letterSpacing: "0.08em", whiteSpace: "nowrap" }}>{label}</span>
      <div style={{ flex: 1, height: 1, background: "#2a3245" }} />
    </div>
  );
}
...
function IconBtn({ children, onClick, title, danger, loading }: { children: React.ReactNode; onClick: () => void; title?: string; danger?: boolean; loading?: boolean }) {
  return (
    <button title={title} onClick={onClick} disabled={loading}
      style={{ padding: "6px 10px", borderRadius: 6, background: danger ? "rgba(239,68,68,0.1)" : "#1c2333", border: `1px solid ${danger ? "rgba(239,68,68,0.2)" : "#2a3245"}`, color: danger ? "#ef4444" : "#8892a4", cursor: "pointer", fontSize: 13 }}>
      {children}
    </button>
  );
}

const labelStyle: React.CSSProperties = { fontSize: 11, color: "#8892a4", fontWeight: 500 };
const inputStyle: React.CSSProperties = { background: "#0d1117", border: "1px solid #2a3245", borderRadius: 6, color: "#e2e8f0", padding: "7px 10px", fontSize: 12, width: "100%" };
const selectStyle: React.CSSProperties = { background: "#0d1117", border: "1px solid #2a3245", borderRadius: 6, color: "#e2e8f0", padding: "7px 10px", fontSize: 12, width: "100%", cursor: "pointer" };
